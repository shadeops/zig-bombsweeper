...
fn interact(board: *GameBoard) void {
    if (!ray.IsCursorOnScreen()) return;
    var x = ray.GetMouseX();
    var y = ray.GetMouseY();
    ...
    for (board.positions) |pos, i| {
        ...
        if (!pos.is_visible) {
            ...
            if (board.neighbouring_bombs[i] > 0) {
                ...
                try std.fmt.formatIntValue(board.neighbouring_bombs[i], "", .{}, fbs.writer());
                var num_str = fbs.getWritten();
                buf[num_str.len] = 0;

                var text_size: i32 = @divFloor(ray.MeasureText(&buf, 20), 2);
                ...
                ray.DrawText(&buf, x * square_size + (25 / 2 - text_size), y * square_size + 3, 20, color);
            }
        }
    }
}
...
fn highlightTile() void {
    ...
    var yellow = ray.Color{ .r = 255, .g = 255, .b = 0, .a = 128 };
    ray.DrawRectangleRec(rect, yellow);
}

fn drawMsg(msg: []const u8) void {
    const text_size = 50;
    var text_width = ray.MeasureText(msg.ptr, text_size);
    var text_x = @divFloor(ray.GetScreenWidth() - text_width, 2);
    var text_y = @divFloor(ray.GetScreenHeight(), 2) - text_size / 2;
    ray.DrawText(msg.ptr, text_x, text_y, text_size, ray.BLACK);
}
...
pub fn main() anyerror!void {
    ...
    while (!ray.WindowShouldClose()) {
        ...
        if (ray.IsCursorOnScreen()) highlightTile();

        if (board.winningState()) {
            drawMsg("For Great Justice!");
        } else if (board.failed) {
            drawMsg("Make Your Time!");
        }

        ray.EndDrawing();

        if (ray.IsKeyPressed('R')) board = try GameBoard.init(num_bombs);
        if (ray.IsKeyPressed('Q')) return;
    }
}
